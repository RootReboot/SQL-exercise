...
-- 1.10 Select all the data from the products, including all the data for each product's manufacturer.
  --Manufacter only brings name because products already has the code has is foreign key.
select
  a.*,
  b.name
from Products a
JOIN Manufacturers B ON(b.code = a.manufacturer);
...
select
  Manufacturers.name
from Manufacturers
INNER join products on Products.Manufacturer = Manufacturers.code
group by
  Manufacturers.name
having
  avg(Products.price) >= 150;
...
-- 1.16 Select the name of each manufacturer along with the name and price of its most expensive product.
  -- I used group by but with only displays one max price for each manufacturers
select
  Manufacturers.name,
  Products.price,
  Products.name
